...
public struct SSHCommandResult: Equatable, Sendable {
    ...
    public init(terminationStatus: Int32, errorText: String) {
        self.terminationStatus = terminationStatus
        self.errorText = errorText
    }
}

public final class SSHProcess: @unchecked Sendable {
    ...
    public static func launchSupervised(
    ...
    public static func run(_ command: SSHCommand) async throws -> SSHCommandResult {
    ...
    /// Runs the command capturing both streams; standard output is appended to
    /// `errorText` after the stderr content. Used by diagnostics for tools
    /// that print their version to stdout on some platforms (e.g. `ssh -V`).
    public static func runCapturingBothStreams(_ command: SSHCommand) async throws -> SSHCommandResult {
        let process = Process()
        let errorPipe = Pipe()
        let outputPipe = Pipe()
        process.executableURL = command.executableURL
