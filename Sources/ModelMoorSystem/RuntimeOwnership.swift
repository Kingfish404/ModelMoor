...
public final class RuntimeOwnership: @unchecked Sendable {
    ...
    public static func acquire(
        ...
        guard ftruncate(descriptor, 0) == 0,
              ...
              identity.withCString({ pointer in write(descriptor, pointer, strlen(pointer)) }) == identity.utf8.count,
    ...
    /// Reads the recorded owner identity without touching the lock, so
    /// read-only surfaces (TUI, doctor) can report the current runtime owner.
    /// The answer may be stale if the owner died without cleanup; treat it as
    /// advisory, never as proof of liveness.
    public static func recordedOwner(lockFileURL: URL) -> String? {
        guard let data = try? Data(contentsOf: lockFileURL) else { return nil }
