...
struct UsageDetailView: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Grid(alignment: .leading, horizontalSpacing: 24, verticalSpacing: 0) {
                    ...
                    ForEach(report.breakdowns) { item in
                        GridRow {
                            Text(modelName(item.routeID))
                                .lineLimit(1)
                            Text(endpointName(item.endpointID))
                                .foregroundStyle(.secondary)
                                .lineLimit(1)
    ...
    private var refreshQuery: UsageRefreshQuery {
        UsageRefreshQuery(query: query, isActive: model.isUIRefreshActive)
    }

    private var selectedPoint: TokenUsageSeriesPoint? {
        guard let selectedTimestamp, let series = report?.series else { return nil }
        return series.min {
    ...
    private func reload(showProgress: Bool) async {
        ...
        let value = await model.tokenUsageReport(
            ...
            to: endDate,
            bucketInterval: timeRange.bucketInterval,
            routeID: routeID,
    ...
    private func modelName(_ id: UUID?) -> String {
        guard let id else { return "Earlier usage" }
        return model.configuration.routes.first(where: { $0.id == id })?.publicModel ?? "Removed model"
    }

    private func endpointName(_ id: UUID?) -> String {
        guard let id else { return "Unknown endpoint" }
        return model.configuration.endpoints.first(where: { $0.id == id })?.name ?? "Removed endpoint"
    }
