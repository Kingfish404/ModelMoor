...
public actor TokenUsageStore {
    ...
    public func report(
        ...
        for record in records ?? [] where record.timestamp >= startDate && record.timestamp <= endDate {
    ...
    private func aggregate(now: Date) -> TokenUsageSnapshot {
        ...
        var result = TokenUsageSnapshot.zero
        for record in records ?? [] where record.timestamp <= now && record.timestamp >= month {
            result.last30Days = addingClamped(result.last30Days, record.tokens)
            if record.timestamp >= day {
                result.lastDay = addingClamped(result.lastDay, record.tokens)
            }
            if record.timestamp >= hour {
                result.lastHour = addingClamped(result.lastHour, record.tokens)
            }
            if record.timestamp >= minute {
                result.lastMinute = addingClamped(result.lastMinute, record.tokens)
            }
        }
        return result
    }
