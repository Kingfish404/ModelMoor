...
struct SSHConnectionDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: connectionID) { oldID, _ in
            dirtyDrafts.unregister(id: .connection(oldID))
            reloadDraft()
            synchronizeDirtyDraft()
        }
        ...
        Section("Used By") {
            ...
            if endpoints.isEmpty {
                ...
                ForEach(endpoints) { endpoint in
                    ...
                    .buttonStyle(.plain)
                    .accessibilityHint("Opens API endpoint details")
                }
            }
        }
    }
    ...
    private func forwards(_ connection: Binding<TunnelConfiguration>) -> some View {
        Section {
            ForEach(connection.mappings) { mapping in
                HStack(spacing: 10) {
                    Toggle(isOn: mapping.enabled) { EmptyView() }
                        .labelsHidden()
                    ...
                    .accessibilityLabel("Remove \(mapping.wrappedValue.name) port forward")
        ...
        Section {
            DisclosureGroup("Advanced SSH Settings", isExpanded: $showsAdvanced) {
                ...
                LabeledContent("Control command") {
                    Text(verbatim: "/usr/bin/ssh -M -N \(connection.wrappedValue.sshHost)")
                        .font(.caption.monospaced())
                        .foregroundStyle(.secondary)
    ...
    private var draftID: DirtyDraftID { .connection(connectionID) }
    ...
    private func synchronizeDirtyDraft() {
        ...
        dirtyDrafts.register(
            id: draftID,
            title: draft.name.isEmpty ? AppLocalization.string("This SSH connection") : "“\(draft.name)”",
            isDirty: isDirty
        ) {
            await model.applyTunnel(draft)
        }
    }
